...
P.S. Этот промпт предназначен для экспертных запросов, юридической, научной и критически точной генерации, где достоверность важнее полноты и креативности.`}[i];if(n){const e=document.createElement("textarea");e.style.position="fixed",e.style.opacity="0",e.value=n,document.body.appendChild(e),e.focus(),e.select(),document.execCommand("copy"),document.body.removeChild(e)}}})})}function y(){const s=document.getElementById("searchToggle"),o=document.querySelector(".search__inner"),i=document.getElementById("search");if(!s||!o||!i)return;document.querySelectorAll(".tag.copy").forEach(t=>{const c=window.getComputedStyle(t).display;t.dataset.origDisplay=c!=="none"?c:"inline-block"});function a(){o.classList.remove("active"),i.value="",document.querySelectorAll(".tag.copy").forEach(t=>{t.style.display=t.dataset.origDisplay})}s.addEventListener("click",()=>{o.classList.contains("active")?a():(o.classList.add("active"),i.focus())}),document.addEventListener("click",t=>{!o.contains(t.target)&&t.target!==s&&a()}),document.addEventListener("keydown",t=>{t.key==="Escape"&&a()});let n=0,e=0,r=!1;document.addEventListener("touchstart",t=>{o.classList.contains("active")&&(n=t.touches[0].clientX,r=!0,o.dataset.isSwiping="true")}),document.addEventListener("touchmove",t=>{const c=document.querySelector(".search__inner.active[data-is-swiping='true']");c&&r&&(e=t.touches[0].clientX,n-e>100&&(a(),r=!1,delete c.dataset.isSwiping))}),document.addEventListener("touchend",()=>{const t=document.querySelector(".search__inner.active[data-is-swiping='true']");t&&(r=!1,delete t.dataset.isSwiping)}),i.addEventListener("input",()=>{const t=i.value.trim().toLowerCase();document.querySelectorAll(".tag.copy").forEach(l=>{const d=l.getAttribute("data-search-tg-password");if(!d)return;d.toLowerCase().includes(t)?l.style.display=l.dataset.origDisplay:l.style.display="none"})})}m(),p(),f(),g(),u(),y();
